...
3. Repo hiện đã được vá thêm một số lỗi source rõ ràng như phần `currency` và các asset cần thiết để `flagd-ui` build qua CI, nhưng nếu CI báo thêm service lỗi build thì nên xử lý tiếp từng service dựa trên log của workflow.
